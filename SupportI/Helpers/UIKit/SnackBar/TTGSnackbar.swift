...
open class TTGSnackbar: UIView {
    ...
    @objc open dynamic var shouldActivateLeftAndRightMarginOnCustom: Bool = false
...
//    }
    ...
    /**
     ...
     - returns: TTGSnackbar instance
     */
    public init(message: String, duration: TTGSnackbarDuration, actionText: String, messageFont: UIFont,
                actionTextFont: UIFont, actionBlock: @escaping TTGActionBlock) {
        super.init(frame: TTGSnackbar.snackbarDefaultFrame)
        self.duration = duration
        self.message = message
        self.actionText = actionText
...
public extension TTGSnackbar {
    ...
    func configShowStep2(superView: UIView) {
    ...
    @objc func show() {
        ...
        if let superView = containerView ?? UIApplication.shared.delegate?.window ?? UIApplication.shared.keyWindow {
            ...
            configShowStep2(superView: superView)
            // Active or deactive
            topMarginConstraint?.isActive = false // For top animation
            leftMarginConstraint?.isActive = self.shouldActivateLeftAndRightMarginOnCustom ? true : customContentView == nil
            rightMarginConstraint?.isActive = self.shouldActivateLeftAndRightMarginOnCustom ? true : customContentView == nil
...
public extension TTGSnackbar {
    ...
    fileprivate func dismissAnimated(_ animated: Bool) {
        ...
        dismissAnimate(animationBlock: animationBlock)
    }

    func dismissAnimate(animationBlock: (() -> Void)?) {
        UIView.animate(withDuration: animationDuration, delay: 0,
                       usingSpringWithDamping: animationSpringWithDamping,
...
private extension TTGSnackbar {
    ...
    func configConstraintVertical() {
        let hConstraints = NSLayoutConstraint.constraints(
            ...
            metrics: nil,
            views: ["iconImageView": iconImageView, "messageLabel": messageLabel, "seperateView": separateView,
                    "actionButton": actionButton, "secondActionButton": secondActionButton])
